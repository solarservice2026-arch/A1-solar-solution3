...
export function agreementDocument(row: RecordValue) {
 ...
 <p class="clause"><b>19. Mutually Agreed Terms of Payment:</b></p><p>The cost of the plant and payment schedule should be mutually discussed and decided between the vendor and consumer. The consumer may opt for milestone-based payment to the vendor and the same shall be included in the agreement.</p>
  <div class="party-grid"><div class="signature-box"><b>First Party (Consumer)</b><p>Name: ${esc(customer.name)}<br>Address: ${address}</p>${row.customer_signature_url ? `<img class="vendor-sign" src="${esc(String(row.customer_signature_url))}" alt="Customer signature" style="height:20mm;max-width:58mm;object-fit:contain;display:block;margin:1mm 0;print-color-adjust:exact;-webkit-print-color-adjust:exact">` : `<div style="height:25mm"></div>`}<div class="line">Signature:<br>Date: ${esc(displayDate)}</div></div><div class="signature-box"><b>Second Party (Vendor)</b><p>Name: A1 SOLAR SOLUTIONS<br>Address: VISHNUPUR KAIJU PATEHPUR<br>VAISHALI BIHA</p><img class="vendor-sign" src="${esc(vendorSign)}" alt="Authorized vendor signature"><div class="line">Signature:<br>Date: ${esc(displayDate)}</div></div></div>
